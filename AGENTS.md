...
### 2.3 認知済み「多形式タクソノミー」は最初からフォルダ + 全収録（table 方式）⭐⭐

§2.2 の on-demand grow は **散発的・単発の文書**に適用するルール。
対して、ある topic が **生産級の「形式（form）」を複数列挙できる成熟したタクソノミー**
だと最初から認識できる場合は、**`table/` と同様に最初から topic フォルダを作り、
人気の生産級形式を一気に全収録する**。単発ファイルを小出しにしない（= 開発期 §0
「一次到位」の精神）。

**「最初から全収録」モードの判定（3 つ全て満たす）**:
1. 生産級の form を **3 つ以上**事前に列挙できる
2. 各 form に §3.6 を満たす **star > 5,000 の OSS 実装**が実在する
3. form 間に「どれをいつ使うか」の**決定軸**がある（= 決定木が書ける）

**収録物（`table/` を踏襲）**:
- `<topic>/README.md` — 索引 + 決定木 + Summary 表（各 form の Best Lib 列、metadata 冒頭）
- `<topic>/NN_<form>.md` — form ごとに 1 ファイル。各々が §3（metadata / 結論先出し /
  伪代码 / Pitfalls）+ §3.6（実測 References ≥1 件、star > 5,000、verified 日付）を完備

**判断例**: JWT 単体は topic ではなく **auth topic の 1 form**。よって `jwt.md` を
散発作成せず、`auth/` を立てて session-cookie / JWT / OAuth2-OIDC / API key /
WebAuthn(Passkeys) 等の人気生産級形式をまとめて全収録する。
